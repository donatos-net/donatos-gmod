...
export function tabShop(
	...
	const allCategories: {
		...
		remoteConfig.value?.goodsCategories.map((c) => ({
			id: c.id,
			name: c.name,
			goods: [],
		})) ?? []
	...
	for (const item of remoteConfig.value?.goods || []) {
		allCategories
			.find((c) => c.id === (item.categoryId ?? -1))
			?.goods.push(item)
	}

	const filteredCategories = allCategories.filter((c) =>
		selectedCategoryId ? selectedCategoryId === c.id : true,
	)
	...
	{
		const navbarContainer = themedUi().panel({
			parent: container,
			color: Color(0, 0, 0, 0),
		})
	...
	for (const group of filteredCategories) {
      ...
      headerContainer.SizeToChildren(false, true)*/
		...
		const itemCardColor = customizedColor(
			donatos.uiConfig?.components?.shopItem,
			'card',
		)
...
async function buyItem(
	id: number,
	variant: { id: string; price: number; duration?: number },
) {
	if (LocalPlayer().Donatos().Balance >= variant.price) {
		...
		if (payUrl)
			gui.OpenURL(
				`${string.Replace(payUrl, '{id}', LocalPlayer().SteamID64())}&openDeposit=${variant.price}`,
			)
	}
}
...
export function itemCard(
	item: {
		id: number
		name: string
		variants?: { id: string; price: number; duration?: number }[]
	},
	panel: DPanel,
	descriptionMarkup: MarkupObject,
	descriptionHeight: number,
) {
	const componentColor = customizedColor(
		donatos.uiConfig?.components?.shopItem,
		'card',
	)
	...
	const description = vgui.Create('DLabel', itemContainer) as DLabel &
		PaintExt<DLabel>
	description.SetText('')
