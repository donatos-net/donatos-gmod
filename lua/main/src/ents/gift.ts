...
const ent = {
	...
	Initialize: SERVER
		? function (this: GiftEnt) {
				this.SetModel(
					donatos.config?.gift?.model ?? 'models/dav0r/hoverball.mdl',
				)
		...
		? function (this, ent: Entity) {
				...
				serverApiRequest('player:claim-item', {
					...
					ply
						.Donatos()
						._sPrint(
							'Вы получили в подарок предмет. Откройте донат-меню для активации.',
						)

					const ownerPlayer = player.GetBySteamID64(this._itemOwner) as
						| Player
						| false
