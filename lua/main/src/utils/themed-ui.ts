import { type ButtonExt, type PaintExt, type PerformLayoutExt, cAlpha, ui } from '@/utils/ui'
...
export class ThemedUi {
  ...
  stylizeBtn(
    ...
    button.Paint = function (this: DButton, w: number, h: number) {
      ...
      draw.RoundedBoxEx(r, 0, 0, w, h, drawColor, ro.topLeft, ro.topRight, ro.bottomLeft, ro.bottomRight)
    }
  ...
  panel({
    ...
    color,
    radius,
  }: { parent?: Panel; classname?: string; color?: Color; radius?: BorderRadiusVariant } = {}) {
    const panel = vgui.Create(classname || 'DPanel', parent) as DPanel & PaintExt<DPanel>
    const bg = color || this.theme.colors.card
    const r = this.borderRadius[radius || 'sm']
  ...
  scrollPanel({ parent, color, radius }: { parent?: Panel; color?: Color; radius?: BorderRadiusVariant } = {}) {
  ...
  label({
    ...
    color,
    font,
  }: { parent?: Panel; text: string; size?: keyof typeof ui.fontSizes; color?: Color; font?: keyof typeof ui.fonts }) {
  ...
  dermaMenu({
    ...
    size,
    options,
  }: { variant?: ButtonVariant; size?: keyof typeof ui.fontSizes; options: DermaMenuOption[] }) {
    const menu = this.stylizeDermaMenu(DermaMenu() as DMenu & DScrollPanel & PaintExt<DMenu & DScrollPanel>, {
      variant,
    })
    ...
    const addOptions = (menu: DMenu, list: typeof options) => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (optionDef.options) {
          ...
          opt = menu.AddOption(optionDef.text, optionDef.onClick) as DMenuOption & PerformLayoutExt<DMenuOption>
        }
  ...
  stylizeDermaMenu(
    ...
    { variant, radius }: { variant?: ButtonVariant; radius?: BorderRadiusVariant },
  ) {
