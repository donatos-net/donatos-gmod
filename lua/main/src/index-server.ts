...
hook.Add(
	'PlayerInitialSpawn',
	donatosHookId('player_initial_spawn'),
	(ply: Player) => {
		if (ply.IsBot()) {
			return
		}
...
hook.Add(
	'PlayerSay',
	donatosHookId('player_say'),
	(ply: Player, text: string) => {
...
concommand.Add('donatos_update', async (ply: Player) => {
	...
	if (dlResult.isError) {
		log.error(
			`Не удалось скачать релиз ${latestRelease.name}: ${dlResult.error}`,
		)
		return
	}
...
async function checkForAddonUpdates() {
	...
	if (donatos.config?.autoUpdate) {
		log.info(
			'Устанавливаю обновление аддона, так как включено авто-обновление.',
		)
