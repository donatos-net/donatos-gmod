...
export const localBundlePath = CLIENT
	? 'donatos/bundle.txt'
	: 'donatos/bundle.server.txt'

export async function fetchAddonReleases(): Promise<Result<AddonApiResponse>> {
	const res = await httpRequest({
		method: 'GET',
		url:
			donatosBootstrap?.addonApiUrl ??
			'https://donatos.net/api/game-server/gmod/addon',
	})
	if (res.isError) {
...
export function getOfflineAddonRelease():
	| AddonApiResponse['releases'][0]
	| undefined {
