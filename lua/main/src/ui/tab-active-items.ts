	...
	const itemsSorted = LocalPlayer()
		...
		.ActiveItems.sort((a, b) => {
			if (a.expires || b.expires) {
				...
				return (
					a.expires.inS / a.expires.durationS -
					b.expires.inS / b.expires.durationS
				)
			}
			return 0
		})
							...
							? [
									{
										text: 'Разморозить',
										onClick: async () => {
											const result = await netMessageToServer(
												'unfreezeActiveItem',
												{ id: i.id },
											)
											if (result.success) {
...
function askFreezeActiveItem(params: {
	id: number
