...
export async function serverApiRequest<T extends keyof serverApiSchema>(
  ...
  const {
    ...
    url: donatos.config.apiEndpoint || 'https://donatos.net/api/game-server/gmod',
    method: 'POST',
    headers: {
  ...
  if (response.code !== 200 && response.code !== 400) {
    log.warn(`Server API request "${action}" failed with code ${response.code}: ${response.body} `)
    return result.err(`HTTP error ${response.code}`)
  }

  const body: { success: false; error: string } | { success: true; data: serverApiSchema[T]['output'] } =
    util.JSONToTable(response.body)
